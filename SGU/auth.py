...
def password_reset(request,
    ...
    if post_reset_redirect is None:
        post_reset_redirect = reverse('sgu:password_reset_done')
    else:
...
def password_reset_confirm(request, uidb64=None, token=None,
    ...
    if post_reset_redirect is None:
        post_reset_redirect = reverse('sgu:password_reset_complete')
    else:
        post_reset_redirect = resolve_url(post_reset_redirect)
    try:
...
class PasswordResetView(PasswordContextMixin, FormView):
    ...
    html_email_template_name = None
    subject_template_name = 'registration/password_reset_subject.txt'
    success_url = reverse_lazy('sgu:password_reset_done')
    template_name = 'registration/password_reset_form.html'
    title = _('Password reset')
    token_generator = default_token_generator
...
class PasswordResetConfirmView(PasswordContextMixin, FormView):
    ...
    success_url = reverse_lazy('sgu:password_reset_complete')
    template_name = 'registration/password_reset_confirm.html'
    title = _('Enter new password')
    token_generator = default_token_generator
